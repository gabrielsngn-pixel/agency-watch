...
function RegistroPage() {
  ...
  useEffect(() => {
    fetch("https://pixel-watch-guardian.lovable.app/api/public/registro/lookup?type=stages")
      .then((r) => r.json())
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(`https://pixel-watch-guardian.lovable.app/api/public/registro/lookup?type=consultant&email=${encodeURIComponent(trimmed)}`);
...
function AgencyPicker({ value, onChange, onNotFound }: { value: AgencyOption | null; onChange: (a: AgencyOption | null) => void; onNotFound?: () => void }) {
  ...
  useEffect(() => {
    ...
    timer.current = setTimeout(async () => {
      try {
        const r = await fetch(`https://pixel-watch-guardian.lovable.app/api/public/registro/lookup?type=agencies&q=${encodeURIComponent(query.trim())}`);
...
async function submit(payload: Record<string, unknown>, file?: File | null) {
  ...
  const res = await fetch("https://pixel-watch-guardian.lovable.app/api/public/registro/submit", { method: "POST", body: form });
