...
BEGIN
  ...
  IF v_agency_ids IS NOT NULL THEN
    FOREACH v_agency IN ARRAY v_agency_ids LOOP
      -- Only delete agencies that were originally created by the Google Forms sync.
      -- Manually registered agencies must remain in the dashboard even when all
      -- form-sourced activities are removed.
      SELECT EXISTS (
